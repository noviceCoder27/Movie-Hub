...
const CreateAnswer = ({isOpen,onClose,threadId,answer,setAnswer}:Props) => {
    ...
    const generateAns = async () => {
        ...
        const response = await axios.post(`http://localhost:3000/user/addAnswer`,answer,options);
    ...
    const generateComment = async () => {
        ...
        const response = await axios.post(`http://localhost:3000/user/addComment`,answer,options);
        setAnswer({thread_id: threadId,content: "",answer_id: null});
        setThread(response.data);
        onClose();
    }
